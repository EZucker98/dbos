...
bot.on("message", async message => {
    ...
    if(guildIDB.premium == true){
        ...
        if (!PremUListed) {
            ...
            if(userListed.bugBuster == true){
                var SbugBuster = true;
            } else {
                var SbugBuster = false;
            }
            ...
            if(message.member.hasPermission("MANAGE_GUILD")){
                const init = await GUserModel.findOneAndUpdate({ id: message.member.id, guildID: message.guild.id }, { username: messageAuthor, profileImage: AuthorImage, serverSuperAdmin: true, messages: MESG, siteAdmin: SiteAdmin, siteModerator: Sitemoderator, sitePartner: Spartner, siteBugBuster: SbugBuster }, { new: true });
            } else if(message.member.hasPermission("ADMINISTRATOR")){
                const init = await GUserModel.findOneAndUpdate({ id: message.member.id, guildID: message.guild.id }, { username: messageAuthor, profileImage: AuthorImage, serverSuperAdmin: true, messages: MESG, siteAdmin: SiteAdmin, siteModerator: Sitemoderator, sitePartner: Spartner, siteBugBuster: SbugBuster }, { new: true });
            } else if(message.member.hasPermission("BAN_MEMBERS")){
                const init = await GUserModel.findOneAndUpdate({ id: message.member.id, guildID: message.guild.id }, { username: messageAuthor, profileImage: AuthorImage, serverSuperAdmin: true, messages: MESG, siteAdmin: SiteAdmin, siteModerator: Sitemoderator, sitePartner: Spartner, siteBugBuster: SbugBuster }, { new: true });
             } else {
                const init = await GUserModel.findOneAndUpdate({ id: message.member.id, guildID: message.guild.id }, { username: messageAuthor, profileImage: AuthorImage, messages: MESG, siteAdmin: SiteAdmin, siteModerator: Sitemoderator, sitePartner: Spartner, siteBugBuster: SbugBuster }, { new: true });
            }
            if (config.danger.debug == true) {
...
bot.on("message", async message => {
    try {
        ...
        var xp = gxp;
        console.log(xp)
        const levelst = await levels.findOneAndUpdate({ guildID: message.guild.id, userID: UID }, { userTag: messageAuthor, userImage: AuthorImage, xp: xp, siteAdmin: Sadmin, siteModerator: Smod, sitePartner: Spartner, siteBugBuster: SbugBuster }, { new: true });
        var cxp = levelst.xp;
        if (config.danger.debug == true) {
